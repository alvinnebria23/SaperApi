...
const generateAndSaveLink = async (req, res) => {
  try {
    ...
    let sanitizedSubIds = [];
    if(subIds?.length > 0){
      sanitizedSubIds = subIds.map(subId => subId.replace(/[^a-zA-Z0-9]/g, ''));
    }
    for(const link of originalUrl){
      const query = getGenerateShortLinkQuery(link, sanitizedSubIds);
    ...
    for(const link of links){
      await saveLink(link.originalUrl, link.shortLink, sanitizedSubIds, shopeeApi.userId);
    }
